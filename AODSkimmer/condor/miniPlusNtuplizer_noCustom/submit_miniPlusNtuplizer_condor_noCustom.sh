...
do
	...
	condor_submit miniPlusNtuplizer_noCustom_config.jdl -append "Arguments = ${sublist_name} ${year} ${nThreads} ${isData} ${isSignal} ${outPath}" -append "transfer_input_files = ${sublist_full}" -append "request_cpus = ${nThreads}"
done

...
do
	...
	condor_submit ntuplizer_config.jdl -append "Arguments = ${sublist_name} ${year} ${nThreads} ${isData} ${isSignal} ${outDirName}" -append "transfer_input_files = ${sublist_full}" -append "request_cpus = ${nThreads}"
done

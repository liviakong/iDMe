...
float DisplacedDileptonsAOD::getDeltaR(float phi1, float eta1, float phi2, float eta2) {
...
bool DisplacedDileptonsAOD::passIsotrackSelection(const pat::IsolatedTrack &track) {
    // Quality cuts:
...
bool DisplacedDileptonsAOD::passPhotonSelection(const reco::Photon &photon) {
    // Quality cuts:
...
bool DisplacedDileptonsAOD::passBaselineSelection(llCandidate llc) {
    // Electron selection:
    if (llc.type == 0) {
...
bool DisplacedDileptonsAOD::passBaselineSelection(trackPair ttp) {
...
void DisplacedDileptonsAOD::findDileptons() {
    ...
    while (1){
        ...
        for (size_t t = 0; t < iT.size(); t++){
            ...
            for (size_t sc = 0; sc < iP.size(); sc++){
                const reco::Photon & photon = (photons).at(iP.at(sc));
